...
class BgUtil extends CommonStyle<BgUtil> {
  ...
  /// Creates a new [BgUtil] with the specified opacity applied.
  ///
  /// Uses Tailwind's color opacity modifier syntax (e.g. `bg-primary/90`).
  /// [opacity] should be between 0.0 and 1.0.
  BgUtil withOpacity(double opacity) {
    // Convert 0.9 -> 90
    final percent = (opacity * 100).round();
    // Append /90 to the existing class (e.g. 'bg-base-100/90')
    return BgUtil('$cssClass/$percent', modifiers: modifiers);
  }
